...
int main(int argc, char** argv) {
    ...
    if(!image_path || !size_kib || !inode_count) {
        usage(argv[0]);
        return 2;
    }
    ...
    root.direct[0] = (uint32_t)sb.data_region_start;
    for(int i = 1; i < 12; i++) {
        root.direct[i] = 0;
    }
    ...
    if(fseek(f, (long)(total_blocks * BS) - 1, SEEK_SET) != 0) {
    ...
    #define WRITE_AT_BLOCK(block_index, buf, len_desc, expected_bytes) do { \
        if(fseek(f, (long)(block_index) * BS, SEEK_SET) != 0) { \
            fprintf(stderr, "fseek failed for %s (block %llu)\n", len_desc, (unsigned long long)(block_index)); \
            return 7; \
        } \
        if(fwrite(buf, 1, expected_bytes, f) != (size_t)expected_bytes) { \
